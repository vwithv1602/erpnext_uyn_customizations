...
def get_data():
	sql = """ (select name as name,item_code as item_code,warehouse as warehouse,purchase_date as purchase_date,"" as material_request,"" as requested_by from `tabSerial No` where warehouse = 'Tech Team - Uyn' and item_group='Laptops' and purchase_date>'2018-07-31' and name not in (select sn.name as name from `tabSerial No` sn inner join `tabMaterial Request Item` mri on mri.serial_no=sn.name inner join `tabMaterial Request` mr on mr.name=mri.parent where sn.warehouse = 'Tech Team - Uyn' and sn.item_group='Laptops' and sn.purchase_date>'2018-07-31' and mr.docstatus=1 and mr.status='Pending' and mr.post_order_status!='Completed'))
	union
	...
	data = []
	for r in res:
		data.append([r.get("name"),r.get("item_code"),r.get("warehouse"),str(r.get("purchase_date")),r.get("material_request"),r.get("requested_by")])
